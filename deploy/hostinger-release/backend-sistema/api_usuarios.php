...
function normalizarPermisosSesion($raw) {
    if (is_string($raw)) {
        $decoded = json_decode($raw, true);
        if (json_last_error() === JSON_ERROR_NONE) {
            $raw = $decoded;
            ...
            try {
                ...
                if ($stmt->execute()) {
                    if ($stmt->affected_rows > 0) {
                        if (isset($_SESSION['usuario']) && is_array($_SESSION['usuario']) && (int)($_SESSION['usuario']['id'] ?? 0) === (int)$id) {
                            $stmtSesion = $mysqli->prepare('SELECT id, usuario, nombre, rol, permisos FROM usuarios WHERE id = ? LIMIT 1');
                            if ($stmtSesion) {
                                $stmtSesion->bind_param('i', $id);
                                $stmtSesion->execute();
                                $rowSesion = $stmtSesion->get_result()->fetch_assoc();
                                $stmtSesion->close();

                                if ($rowSesion) {
                                    $rowSesion['permisos'] = normalizarPermisosSesion($rowSesion['permisos'] ?? '[]');
                                    $_SESSION['usuario'] = $rowSesion;
                                }
                            }
                        }
