...
function pagar_honorario_asegurar_columna(mysqli $conn, string $tabla, string $columna, string $definicion): void
{
...
try {
    ...
    pagar_honorario_asegurar_columna($conn, 'egresos', 'fuente_fondos', "VARCHAR(30) NOT NULL DEFAULT 'clinica'");
    pagar_honorario_asegurar_columna($conn, 'egresos', 'tercero_nombre', 'VARCHAR(150) NULL');
    pagar_honorario_asegurar_columna($conn, 'egresos', 'referencia_pago', 'VARCHAR(150) NULL');
    pagar_honorario_asegurar_columna($conn, 'honorarios_medicos_movimientos', 'fuente_pago_medico', "VARCHAR(30) NOT NULL DEFAULT 'clinica'");
    pagar_honorario_asegurar_columna($conn, 'honorarios_medicos_movimientos', 'referencia_pago_medico', 'VARCHAR(150) NULL');
    ...
    $stmtCaja->close();
    if (!$caja) {
        throw new RuntimeException('Debes tener una caja abierta para pagar honorarios');
    }
    $cajaId = (int)$caja['id'];
    ...
    $stmtActualizar = $conn->prepare("UPDATE honorarios_medicos_movimientos SET estado_pago_medico = 'pagado', fecha_pago_medico = CURDATE(), metodo_pago_medico = ?, fuente_pago_medico = ?, referencia_pago_medico = ?, observaciones = ?, caja_id = ? WHERE id = ? AND estado_pago_medico = 'pendiente'");
    ...
    foreach ($honorarios as $honorario) {
        if (strtolower((string)$honorario['estado_pago_medico']) !== 'pendiente') {
            throw new RuntimeException('El honorario #' . (int)$honorario['id'] . ' ya fue procesado');
        }
        ...
        $stmtActualizar->bind_param('ssssii', $metodoPago, $fuenteFondos, $referenciaPago, $observaciones, $cajaId, $honorarioId);
        $stmtActualizar->execute();
        if ($stmtActualizar->affected_rows !== 1) {
            throw new RuntimeException('No se pudo actualizar el honorario #' . $honorarioId);
        }
    ...
    echo json_encode(['success' => true, 'message' => 'Honorarios pagados correctamente', 'total' => $total, 'caja_id' => $cajaId]);
} catch (Throwable $e) {
    try { $conn->rollback(); } catch (Throwable $ignored) {}
    error_log('api_pagar_honorario_medico.php: ' . $e->getMessage());
    http_response_code(422);
