...
export default function ConsultButton({ nombreServicio, whatsappNumero }) {
  ...
  const handleConsultar = async () => {
    if (!whatsappNumero) {
      Swal.fire({
        icon: 'warning',
        title: 'No disponible',
        text: 'El número de WhatsApp no está configurado en el sistema.',
      })
      return
    }
    ...
    try {
      const mensaje = `Hola, me interesa consultar sobre: ${nombreServicio}`
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'No se pudo abrir WhatsApp. Intenta de nuevo.',
      })
    } finally {
      setLoading(false)
    }
