    ...
    try {
      ...
      if (additionsAfterReductions.length > 0) {
        const resp = await fetch(`${BASE_URL}api_cobro_actualizar.php`, {
          ...
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ cobro_id: Number(cobroId), servicio_tipo: 'operacion', items: additionsAfterReductions })
        });
  ...
  const cotizar = async () => {
    ...
    const payload = cotizacionId
      ? {
          ...
          detalles,
          total,
          motivo: 'Edición de cotización desde cotizador de Operaciones'
        }
      : {
