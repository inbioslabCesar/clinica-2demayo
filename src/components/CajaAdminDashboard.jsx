...
export default function CajaAdminDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 bg-white rounded-xl shadow-lg">
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)}>
        <div className="p-4">
          ...
          <AperturaCajaForm
            usuario={usuario}
            onApertura={async () => {
              // cerrar modal y recargar resumen sin recargar la página
              setShowModal(false);
              await fetchResumen();
            }}
          />
      ...
      {/* ...existing code... (Resumen Diario, Cards de Recepcionistas) */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {resumen.por_servicio && resumen.por_servicio.length > 0 ? (
              resumen.por_servicio.map((serv, idx) => {
                let color = "purple";
                let icon = (
                  <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#a855f7"/><text x="12" y="16" textAnchor="middle" fontSize="10" fill="#fff">{serv.tipo_ingreso[0]}</text></svg>
                );
                let desc = "Servicio";
                if (serv.tipo_ingreso.toLowerCase().includes("consulta")) {
                  color = "blue";
                  icon = (
                    <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#3b82f6"/><text x="12" y="16" textAnchor="middle" fontSize="10" fill="#fff">C</text></svg>
                  );
                  desc = "Consultas";
                } else if (serv.tipo_ingreso.toLowerCase().includes("laboratorio")) {
                  color = "green";
                  icon = (
                    <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#22c55e"/><text x="12" y="16" textAnchor="middle" fontSize="10" fill="#fff">L</text></svg>
                  );
                  desc = "Laboratorio";
                } else if (serv.tipo_ingreso.toLowerCase().includes("farmacia")) {
                  color = "yellow";
                  icon = (
                    <svg width="32" height="32" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#fbbf24"/><text x="12" y="16" textAnchor="middle" fontSize="10" fill="#fff">F</text></svg>
                  );
                  desc = "Farmacia";
                }
                return (
                  <div key={idx} className={`rounded-xl shadow bg-white border-t-4 border-${color}-400 p-4 flex flex-col items-start gap-2`}>
                    ...
                    <div className={`text-xs text-${color}-600 flex items-center gap-1`}>
                      <svg width="16" height="16" fill="none" viewBox="0 0 24 24"><circle cx="8" cy="8" r="6" fill={`#${color === 'purple' ? 'a855f7' : color === 'blue' ? '3b82f6' : color === 'green' ? '22c55e' : 'fbbf24'}`}/></svg>
                      {desc}
                    </div>
                  </div>
                );
      ...
      {usuario && usuario.rol === "administrador" && (
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {(() => {
              // Filtro robusto para rol recepcionista
              const cajasRecep = resumen.cajas_resumen
                ? resumen.cajas_resumen.filter(caja => {
                    // Buscar rol en varias propiedades y sin importar mayúsculas
