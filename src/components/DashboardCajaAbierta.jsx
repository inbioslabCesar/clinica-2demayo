...
export default function DashboardCajaAbierta({ cajaActual, resumenHoy, onActualizar }) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200 rounded-xl p-4 md:p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-bold text-green-700">Caja Abierta</h2>
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 md:gap-6 text-green-600 mt-1 text-xs sm:text-sm">
                ...
                <div className="flex items-center gap-1">
                  <FaMoneyBillWave />
                  <span>Inicial: {formatearMonto(cajaActual.monto_apertura)}</span>
                </div>
                <div className="flex items-center gap-1">
                  <FaCalendarDay />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-green-500 transform hover:scale-105 transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm font-medium uppercase tracking-wide">Efectivo</p>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500 transform hover:scale-105 transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm font-medium uppercase tracking-wide">Tarjetas</p>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-purple-500 transform hover:scale-105 transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm font-medium uppercase tracking-wide">Digital</p>
              <p className="text-3xl font-bold text-purple-600 mt-2">
                {formatearMonto(resumenHoy?.total_transferencias)}
              </p>
              <p className="text-purple-500 text-sm mt-1">📱 Yape/Plin/Transfer</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-yellow-500 transform hover:scale-105 transition-all">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm font-medium uppercase tracking-wide">Total del Día</p>
              <p className="text-3xl font-bold text-yellow-600 mt-2">
                {formatearMonto(resumenHoy?.total_dia)}
              </p>
              <p className="text-yellow-500 text-sm mt-1">📊 Todos los ingresos</p>
            </div>
      ...
      {ingresosPorArea.length > 0 && (
        <div className="bg-white rounded-xl shadow-lg p-6 h-96 dashboard-scroll-section">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-sm text-gray-500">{ingresosPorArea.length} áreas activas</span>
          </div>
          
          <div className="overflow-y-auto h-72 pr-2">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {ingresosPorArea.map((area, index) => (
                <div key={index} className="bg-gray-50 rounded-lg p-4 border hover:shadow-md transition-all">
                  <div className="flex items-center justify-between mb-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${obtenerColorTipo(area.tipo_ingreso)}`}>
              ...
              {ultimasTransacciones.map((transaccion, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${obtenerColorTipo(transaccion.tipo_ingreso)}`}>
                          {transaccion.tipo_ingreso}
                        </span>
                        <span className="text-sm text-gray-600">{transaccion.area}</span>
                      </div>
                      <p className="text-sm text-gray-800 mt-1">{transaccion.descripcion}</p>
                      {transaccion.paciente_nombre && (
                        <p className="text-xs text-gray-500">Paciente: {transaccion.paciente_nombre}</p>
                      )}
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500">{formatearTiempo(transaccion.hora)}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <p className="text-green-100 text-sm">Análisis detallado de ingresos</p>
        </button>
      </div>
    </div>
  );
